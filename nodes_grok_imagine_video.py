...
from .config import API_PATHS, DEFAULT_MODELS, get_config, get_current_base_url


GROK_IMAGINE_VIDEO_DURATION = ["6", "10", "15", "30"]
GROK_IMAGINE_VIDEO_ASPECT_RATIOS = ["16:9", "9:16", "1:1", "3:2", "2:3"]
GROK_IMAGINE_VIDEO_RESOLUTIONS = ["480p", "720p"]
GROK_IMAGINE_VIDEO_AFTER_CONTROL = ["randomize", "preserve", "loop"]
...
class RelayGrokImagineVideo:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "prompt": ("STRING", {"multiline": True}),
                "model": (["grok-imagine-video", "grok-imagine-video-1.5-preview"], {"default": "grok-imagine-video"}),
                "api_key": ("STRING", {"default": ""}),
                "resolution": (GROK_IMAGINE_VIDEO_RESOLUTIONS, {"default": "720p"}),
                "aspect_ratio": (GROK_IMAGINE_VIDEO_ASPECT_RATIOS, {"default": "16:9"}),
            },
            "optional": {
                "image1": ("IMAGE", {"forceInput": True}),
                "image2": ("IMAGE", {"forceInput": True}),
                "image3": ("IMAGE", {"forceInput": True}),
                "duration": (GROK_IMAGINE_VIDEO_DURATION, {"default": "10"}),
                "after_control": (GROK_IMAGINE_VIDEO_AFTER_CONTROL, {"default": "randomize"}),
            },
        }

    RETURN_TYPES = (IO.VIDEO, "STRING", "STRING", "STRING")
    RETURN_NAMES = ("video", "task_id", "response", "video_url")
    FUNCTION = "generate_grok_imagine_video"
    ...
    def _image_to_media_ref(self, image_tensor):
        data_uri = self._image_to_base64_uri(image_tensor)
        if not data_uri:
            return None
        return {"url": data_uri, "type": "image", "mime_type": "image/png"}
    ...
    def _submit(self, base_url, api_key, model, prompt, resolution, aspect_ratio, duration, after_control, images):
        ...
        if after_control and after_control != "randomize":
            payload["after_control"] = after_control
        ...
        for item in images:
            ...
            elif self._is_url_like(item) or str(item).startswith("data:image/"):
                ref_images.append({"url": item, "type": "image", "mime_type": "image/png"})
        ...
        if resp.status_code != 200:
            self._err("grok-imagine-video create error: " + str(resp.status_code) + " - " + resp.text[:500])
        raw = self._response_json(resp, "grok-imagine-video create " + url)
        task_id = raw.get("task_id") or raw.get("id") or raw.get("request_id") or self._first_string(raw, ["task_id", "id", "request_id"])
        if not task_id:
            self._err("grok-imagine-video create response missing task id")
    ...
    def _query(self, base_url, api_key, task_id):
        ...
        if resp.status_code != 200:
            self._err("grok-imagine-video query error: " + str(resp.status_code) + " - " + resp.text[:500])
        raw = self._response_json(resp, "grok-imagine-video query " + url)
    ...
    def _poll(self, base_url, api_key, task_id, poll_interval, max_wait_time):
        ...
        while True:
            ...
            if status in ("fail", "failed", "error", "cancel", "cancelled", "canceled"):
                self._err("grok-imagine-video task failed: " + json.dumps(raw, ensure_ascii=False)[:500])
            if video_url and not status:
                return video_url, raw
            if time.time() - start >= max_wait_time:
                hint = " last_status=" + last_status if last_status else ""
                self._err("grok-imagine-video timeout after " + str(max_wait_time) + "s" + hint)
            time.sleep(max(1, int(poll_interval)))
        return "", last_raw

    def generate_grok_imagine_video(self, prompt, model, api_key, resolution, aspect_ratio, duration, image1=None, image2=None, image3=None, after_control="randomize"):
        base_url = get_current_base_url()
        prompt = self._normalize_prompt(prompt)
        images = []
        for image in (image1, image2, image3):
            if image is not None:
                images.append(self._image_to_media_ref(image))
        api_key = str(api_key or "").strip()
        if not api_key:
            self._err("API key not found. Please fill api_key first.")

        task_id, submit_raw, payload = self._submit(base_url, api_key, model, prompt, resolution, aspect_ratio, duration, after_control, images)
        if isinstance(submit_raw, dict):
            submit_raw = self._unwrap_payload(submit_raw)
        video_url, query_raw = self._poll(base_url, api_key, task_id, 5, 600)
