"""Combined LLAI Doubao Seedream 4.5 text-to-image and image-to-image node."""

from .doubao_seedream import (
    ALL_RATIO_OPTIONS,
    RATIO_OPTIONS_2K,
    RESPONSE_FORMATS,
    SIZE_LEVELS,
...
from .doubao_seedream_45_i2i import MAX_IMAGES, LLDoubaoSeedream45ImageToImage


REFERENCE_IMAGE_KEYS = [f"参考图{i}" for i in range(1, MAX_IMAGES + 1)]


class LLDoubaoSeedream45:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "size": (
                    SIZE_LEVELS,
                    {"default": "2K", "tooltip": "选择 2K 或 4K 分辨率档位"},
                ),
                "watermark": (
                    "BOOLEAN",
    ...
    def generate(
        ...
        **kwargs,
        ...
        if not connected:
            return LLDoubaoSeedream45TextToImage().generate(
                prompt=prompt,
                size=size,
                watermark=watermark,
                ...
                seed=seed,
                timeout=timeout,
                ratio=ratio or RATIO_OPTIONS_2K[0],
            )
        ...
        print(f"[LLAI] Doubao Seedream 4.5 检测到 {len(connected)} 个参考图输入，切换为图生图")
        return LLDoubaoSeedream45ImageToImage().generate(
            参考图1=first_image,
            prompt=prompt,
            size=size,
            ratio=ratio or RATIO_OPTIONS_2K[0],
